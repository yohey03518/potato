...
1. **Given** 股票昨日收盤價高於月均線（符合做多觀察條件）, **When** 今日盤中5分K收盤價由下往上突破20MA（5分月K）, **Then** 系統應產生「買入」訊號。
2. **Given** 持有多單部位, **When** 盤中5分K收盤價由上往下及跌破20MA, **Then** 系統應產生「賣出」訊號。
3. **Given** 股票昨日收盤價低於月均線（符合做空觀察條件）, **When** 今日盤中5分K收盤價由上往下跌破20MA, **Then** 系統應產生「放空」訊號。
4. **Given** 5分K線尚未收盤（時間未到）, **When** 盤中價格暫時穿越20MA, **Then** 系統不應產生任何訊號（需等待K線確認）。
...
- **FR-001**: 系統必須能讀取並管理「觀察標的清單」。
- **FR-002**: 系統必須能接收台股即時行情數據，包含 Tick（逐筆成交）、Bid/Ask（五檔掛單）與 KLine（K線資料）。

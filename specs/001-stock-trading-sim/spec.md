...
- Q: 進場前檢查的「停損距離」具體如何定義？ → A: 預計下單金額與當前 5 分K線 20MA 的價格差距不可超過 5%。
...
- **FR-011**: 系統必須實作複合式風控邏輯：(1) 進場前檢查潛在停損幅度（預計下單價與 20MA 差距不得 > 5%）；(2) 獲利達 5% 時觸發 50% 部分平倉；(3) 剩餘部位依技術訊號或收盤強平出場。

- **FR-012**: 系統必須支援使用者設定「交易手續費率」與「證券交易稅率」，並在計算損益時自動扣除相關成本（含手續費折讓若有）。
